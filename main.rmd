...
<!-- for '\cancel' MathJax package in html version of this document -->
...
Вектор $X = (X_{1}, \dots, X_{n})^{T}$ называется ***случайным***, если $X_{1}, \dots, X_{n}$ 
--- *случайные величины* (далее **с.в**), определенные на одном вероятностном пространстве.
...
\fbox{$M[X \varphi(Y) | Y] = \varphi(Y)M[X \mid Y]$}

**\underline{Доказательство}**

$M[\varphi(Y) X \mid Y] = \int\limits_{- \infty}^{+ \infty} \varphi(Y)X f_{X \mid Y}(x \mid Y)dx = \varphi(Y) \int\limits_{- \infty}^{+ \infty} x
f_{X \mid Y} (x \mid Y)dx =$

$= \varphi(Y) M[X \mid Y]$. $\blacksquare$
...
<!-- check power of 'varphi(Y)..' -->
$M[(X - \hat{X})\varphi(Y)^{T}] = 0$

где $\hat{X} = M[X \mid Y]$

### Доказательство {-}

$M[(X - \hat{X})\varphi(Y)^{T}] = M[X \varphi(Y)^{T}] - M[M[X \mid Y] \varphi(Y)^{T}] =$

= по [Свойству 2](#attr-1-2) $= M[X \varphi(Y)^{T}] - M[M[X \varphi(Y)^{T} \mid Y]] =$ по [Свойству 5](#attr-1-5) $=
M[X \varphi(Y)^{T}] - M[X \varphi(Y)^{T}]
= 0$. $\blacksquare$
...
***Оценкой $X$ по наблюдениям $Y$*** называется любая измеримая функция $\varphi(Y)$.
...
Поскольку по определению $\tilde{X} - \hat{X} = \varphi(Y)$, то в силу [Леммы 2.1](#lemma-2-1) $M[(X - \hat{X})^{T}(\tilde{X} - \hat{X})] = 0$.
...
Тогда в силу [Леммы 1.4, п.2](#lemma-1-4) $X - \mu(Y)$ и $Y$ **независимы**. Построим характеристическую функцию условного распределения $X$
относительно $Y$:
...
в силу [Свойства 2 УМО](#attr-1-2) и *независимости* $X - \mu(Y)$ и $Y$
...
= 0$, т.к. $\sum\limits_{n = 1}^{\infty} P(A_{n}) < \infty$

2. $P(B) = \dots = \lim\limits_{N \to \infty} P(\sum\limits_{n = N}^{\infty} A_{n}) = 
\lim\limits_{N \to \infty} (1 - P(\prod\limits_{n = N}^{\infty} \overline{A}_{n})) = 
1 - \lim\limits_{N \to \infty} P(\prod\limits_{M = N}^{\infty} \prod\limits_{n = N}^{M} \overline{A}_{n})
\overset{*}{=}$

т.к. $\prod\limits_{n = N}^{M + 1} \overline{A}_{n} \subset \prod\limits_{n = N}^{M} \overline{A}_{n}$, то
по *аксиоме непрерывности*

$\overset{*}{=} 1 - \lim\limits_{N \to \infty} \lim\limits_{M \to \infty} P(\prod\limits_{n = N}^{M}
\overline{A}_{n}) = 1 - \lim\limits_{N \to \infty} \lim\limits_{M \to \infty} \prod\limits_{n = N}^{M}
...
***Выборочным средним*** $\overline{X}_{n}$ выборки $Z_{n}$ называется $\overline{X}_{n} = \frac{1}{n}
...
Тогда $\overline{X}_{n} \overset{\text{с.к.}}{\longrightarrow} m_{X}, \overline{X}_{n} 
\overset{P}{\longrightarrow} m_{X}$

### Доказательство {-}

$M[|\overline{X}_{n} - m_{X}|^{2}] = M[|\overline{X}_{n} - M[\overline{X}_{n}]|^{2}] = 
D[\overline{X}_{n}] = \frac{1}{n^{2}} D[\sum\limits_{k = 1}^{n} X_{k}] = 
\frac{n D[X_{1}]}{n^{2}} \overset{n \to \infty}{\longrightarrow} 0$

Т.е. по определению $\overline{X}_{n} \overset{\text{с.к.}}{\longrightarrow} m_{X}$.

С учетом [Леммы 3.3](#lemma-3-3) $\overline{X}_{n} \overset{P}{\longrightarrow} m_{X}$. $\blacksquare$

## Теорема 2 (Закон Больших Чисел Колмогорова) {#theorem-4-2}

Пусть $Z_{n}$ --- *однородная* выборка, $M[X_{k}] = m_{X} < \infty$.

Тогда $\overline{X}_{n} \overset{\text{п.н.}}{\longrightarrow} m_{X}$

## Замечание {-}
Т.о. для *однородной* выборки $\overline{X}_{n}$ сходится **почти наверное** и **по вероятности** к $m_{X}$,
...
Тогда $\overline{X}_{n} \overset{\text{с.к.}}{\longrightarrow} m_{X}, 
\overline{X}_{n} \overset{P}{\longrightarrow} m_{X}$.

### Доказательство {-}

$M[|\overline{X}_{n} - m_{X}|^{2}] = M[|\overline{X}_{n} - M[\overline{X}_{n}]|^{2}] = D[X_{n}] =
\frac{1}{n^{2}} \sum\limits_{k = 1}^{n} D_{k} \leqslant \frac{n D_{max}}{n^{2}} = 
\frac{D_{max}}{n} \overset{n \to \infty}{\longrightarrow} 0$

Т.о. $\overline{X}_{n} \overset{\text{с.к.}}{\longrightarrow} m_{X}$. Тогда в силу [Леммы 3.3](#lemma-3-3)
$\overline{X}_{n} \overset{P}{\longrightarrow} m_{X}$. $\blacksquare$

## Следствие 1

Пусть $Z_{n}$ --- *неоднородная* выборка, $D[X_{k}] = D_{k} \leqslant D_{max} < \infty, k \in \N$.

Тогда $\overline{X}_{n} - \frac{1}{n} \sum\limits_{k = 1}^{n} m_{X_{k}} \overset{\text{с.к.}}{\longrightarrow}
0, \overline{X}_{n} - \frac{1}{n} \sum\limits_{k = 1}^{n} m_{X_{k}} \overset{P}{\longrightarrow} 0$.

### Доказательство {-}

$\overline{X}_{n} - \frac{1}{n} \sum\limits_{k = 1}^{n} m_{X_{k}} = 
\frac{1}{n} \sum\limits_{k = 1}^{n} (X_{k} - m_{X_{k}}) = \frac{1}{n} \sum\limits_{k = 1}^{n} Y_{k}$,
где $M[Y_{k}] = 0, D[Y_{k}] = D[X_{k}] \leqslant D_{max} < \infty$

Тогда $\overline{Y}_{k}$ удовлетворяет условиям [Теоремы 4.3](#theorem-4-3). $\blacksquare$
...
$$\overline{X}_{n} \overset{\text{п.н.}}{\longrightarrow} m_{X}$$
...
$$\frac{N(A)}{N} = \frac{1}{N} \sum\limits_{k = 1}^{N} X_{k} 
\overset{\text{п.н.}}{\longrightarrow} P(A)$$ 
...
Тогда $X_{n} + Y_{n} \overset{d}{\longrightarrow} X + C$
...
$X_{n} + C = \tilde{X}_{n} \overset{d}{\longrightarrow} X + C = \tilde{X}$

Получаем $X_{n} + Y_{n} = \tilde{X}_{n} + \tilde{Y}_{n} \overset{d}{\longrightarrow} \tilde{X} = X + C$.
$\blacksquare$
...
Т.к. $X_{n} \sim Bi(n; p)$, то существуют **независимые** $\tilde{X}_{1}, \dots, \tilde{X}_{n} \sim
Be(p)$ такие, что

$$X_{n} = \sum\limits_{k = 1}^{n} \tilde{X}_{k}, M[\tilde{X}_{k}] = p = m_{X}, D[\tilde{X}_{k}] =
p(1 - p) = \sigma_{X}^{2}$$

Тогда  в силу [Теоремы 1](#theorem-5-1):

$\frac{\sum\limits_{k = 1}^{n} \tilde{X}_{k} - nm_{X}}{\sigma_{X}\sqrt{n}} \overset{d}{\longrightarrow}
N(0; 1)$. $\blacksquare$
...
С.в. $\varphi(Z_{n})$, где $\varphi : S \to \R$ --- *измерима*, называется ***статистикой***.
...
***Выборочным  квантилем*** уровня $\alpha \in (0; 1)$ называется *порядковая статистика* 
$X^{([n\alpha] + 1)}$
...
***Выборочной функцией распределения*** называется статистика $\hat{F}_{n}(x)$:
...
Фактически $\hat{F}_{n}(x)$ --- **частота события** $\{X \leqslant x\}$, которая используется 
...
plotter <- function(data) {
    ...
    p <- ggplot(data.frame(x = c(data[first, 1], dash_data[1, 1]), f = c(data[first,
        2], dash_data[1, 2])), mapping = aes(x = x, y = f)) + geom_hline(linewidth= 1, yintercept = 0) +
        geom_line() + geom_point(data.frame(x = c(dash_data[1, 1]), f = c(dash_data[1,
        2])), mapping = aes(x = x, y = f), shape = 21, size = 3, fill = "white")
    j <- 1
    m <- 2
    while (j <= length(dash_data[, 1])) {
        data_tmp <- data.frame(x = c(dash_data[j, 1], dash_data[j + 1, 1]), f = c(0,
            dash_data[j + 1, 2]))
    ...
    p <- p + geom_line(data.frame(x = c(dash_data[length(dash_data[, 1]), 1], data[last,
        ...
        y = f))
    p <- p + scale_x_continuous(breaks = c(dash_data[1, 1], dash_data[3, 1], dash_data[5,
        1], dash_data[length(dash_data[, 1]) - 2, 1], dash_data[length(dash_data[,
        1]), 1]), labels = c(TeX("$X^{(1)}$"), TeX("$X^{(2)}$"), TeX("$X^{(3)}$"),
        TeX("$X^{(n-1)}$"), TeX("$X^{(n)}$")))
    p <- p + scale_y_continuous(breaks = c(dash_data[2, 2], dash_data[4, 2], dash_data[length(dash_data[,
        1]) - 1, 2], 1), labels = c(TeX("$\\frac{1}{n}$"), TeX("$\\frac{2}{n}$"),
        TeX("$\\frac{n - 1}{n}$"), TeX("$1$")))
...
## Свойства $\hat{F}_{n}(x)$ 

1. \fbox{$n \cdot \hat{F}_{n}(x) \sim Bi(n; F(x))$}

2. \fbox{$M[\hat{F}_{n}(x)] = F(x)$}

3. \fbox{$\sup\limits_{x \in \R}|\hat{F}_{n}(x) - F(x)| \overset{\text{п.н.}}{\longrightarrow} 0$}
***(Теорема Гливенко - Кантелли)***

4. \fbox{$M[(\hat{F}_{n}(x) - F(x))^{2}] = \frac{F(x)(1 - F(x))} 
{n} \leqslant \frac{1}{4n}$}

5. \fbox{$|\hat{F}_{n}(x) - F(x)| \overset{\text{с.к.}}{\longrightarrow} 0$}

6. \fbox{$\frac{\hat{F}_{n}(x) - F(x)}{\sqrt{F(x)(1 - F(x))}} \sqrt{n}
\overset{d}{\longrightarrow} N(0; 1)$}
(Следует из теоремы [Муавра - Лапласа](#cons-5-1))

## Замечание {-}

$\hat{F}_{n}(x)$ при увеличении $n$ *равномерно приближается* к $F(x)$, при этом точность приближения можно
...
$\hat{p}_{k} = \frac{n_{k}}{n}$, где $n_{k}$ --- число элементов выборки, попавших в 
$[t_{k};  t_{k + 1}), k = \overline{0, l}$

Заметим, что $\hat{p}_{0} = \hat{p}_{l} = 0$.
...
\begin{cases}
    ...
    \hat{p}_{k}, & x \in (t_{k}; t_{k + 1}), k = \overline{1, l - 1} \\
\end{cases}$$
...
$$\hat{f}_{n}(x) \overset{P}{\longrightarrow} f_{X}(x)$$
...
$\hat{\nu}_{r}(n) = \frac{1}{n} \sum\limits_{k = 1}^{n} X_{k}^{r}$ 

И

$\hat{\mu}_{r}(n) = \frac{1}{n} \sum\limits_{k = 1}^{n} (X_{k} - \hat{\nu}_{1}(n))^{r}$

## Определение 2

***Выборочным средним*** и ***выборочной дисперсией*** называются соответственно статистики:

$\overline{X}_{n} = \hat{\nu}_{1}(n) = \frac{1}{n} \sum\limits_{k = 1}^{n} X_{k}$ 

И

$\hat{d}_{X}(n) = \hat{S^{2}}(n) = \hat{\mu}_{2}(n) = 
...
$$\hat{r}_{XY} = \frac{\sum\limits_{k = 1}^{n} (X_{k} - \overline{X}_{n})(Y_{k} - \overline{Y}_{k})}
{n\sqrt{\hat{d}_{X} \cdot \hat{d}_{Y}}}$$
...
$M[\hat{\nu}_{r}(n)] = \frac{1}{n} \sum\limits_{k = 1}^{n} M[X_{k}^{r}] = \frac{1}{n} n \nu_{r}$.
$\blacksquare$

2. \fbox{Если $M[X^{r}] < \infty$, то $\hat{\nu}_{r}(n) \overset{\text{п.н.}}{\longrightarrow} \nu_{r}$}

**\underline{Доказательство}**

3. \fbox{Если $M[X^{r}] < \infty$, то $\hat{\mu}_{r}(n) \overset{\text{п.н.}}{\longrightarrow} \mu_{r}$}

**\underline{Доказательство}**

$\hat{\mu}_{r}(n) = \frac{1}{n} \sum\limits_{k = 1}^{n} (X_{k} - \overline{X}_{n})^{r} = 
\frac{1}{n} \sum\limits_{k = 1}^{n} \sum\limits_{i = 0}^{r} C_{r}^{i} X_{k}^{i}( - \hat{X}_{n})^{r - i}=
\sum\limits_{i = 0}^{r} C_{r}^{i} ( - \hat{X}_{n})^{r - i} (\frac{1}{n} \sum\limits_{k = 1}^{n}
X_{k}^{i}) = \sum\limits_{i = 0}^{r} C_{r}^{i} ( - \hat{X}_{n})^{r - i} \hat{\nu}_{i}(n)
\underset{\text{св-во (2)}}{\overset{\text{п.н.}}{\longrightarrow}}
\sum\limits_{i = 0}^{r} C_{r}^{i} (- \nu_{1})^{r - i} \hat{\nu}_{i} = 
\dots = \mu_{i}$. $\blacksquare$

4. \fbox{$D[\overline{X}_{n}] = \frac{1}{n}D[X]$}

**\underline{Доказательство}**

С учетом *независимости* $X_{1}, \dots, X_{n}$

$D[X\overline{X}_{n}] = D[\frac{1}{n} \sum\limits_{k = 1}^{n} X_{k}] = \frac{1}{n^{2}}
\sum\limits_{k = 1}^{n} D[X_{k}] = \frac{n D[X]}{n^{2}}$. $\blacksquare$

5. \fbox{$M[\hat{d}_{X}(n)] = \frac{n - 1}{n}D[X]$}

**\underline{Доказательство}**

$M[\hat{d}_{X}(n)] = \frac{1}{n} \sum\limits_{k = 1}{n} M[(X_{k} - \overline{X}_{n})^{2}] =
M[(X_{1} - \overline{X}_{n})^{2}] = D[X_{1} - \overline{X}_{n})^{2}] = 
D[\frac{n - 1}{n} X_{1} - \sum\limits_{k = 2}{n} \frac{1}{n}\overline{X}_{k}] 
\overset{\text{н / з}}{=} 
\frac{n - 1}{n}D[X_{1}] + \frac{1}{n^{2}} \sum\limits_{k = 2}^{n} D[X_{k}] =
D[X] \frac{n^{2} -2n + 1 + n - 1}{n^{2}} = D[X] \cdot \frac{n - 1}{n}$. $\blacksquare$

6. \fbox{$\frac{\overline{X}_{n} - m_{X}}{\sigma_{X}} \sqrt{n} \overset{d}{\longrightarrow} 
N(0; 1)$}
...
7. \fbox{$\frac{\hat{d}_{X}(n) - \sigma_{X}^{2}}{\sqrt{\mu_{4} - \mu_{2}}} \sqrt{n}
\overset{d}{\longrightarrow} N(0; 1)$}
...
$\hat{d}_{X} = \frac{1}{n} \sum\limits_{k = 1}^{n}(X_{k} - \overline{X}_{n})^{2}$, 

то $\frac{n \hat{d}_{X}}{\sigma_{X}^{2}} \sim \chi^{2}(n - 1)$.
...
set.seed(1)
x <- data.frame(rt(5000, df = 5, ncp = 1) * 3 + 45)
names(x) <- c("test.data")
...
$$\frac{\overline{X}_{n} - m_{X}}{\sqrt{\hat{d}_{X}}} \cdot \sqrt{n - 1} \sim t(n - 1)$$
...
p <- p + ggtitle("") + xlab(label = "x") + ylab(label = TeX("$f_{V}(x, n, m)$")) +
    theme(axis.text = element_text(colour = "red"), axis.title = element_text(colour = "blue"))
...
$$V = \frac{\frac{1}{n - 1} \sum\limits_{k = 1}{n}(X_{k} - \overline{X}_{n})^{2}}
{\frac{1}{m - 1} \sum\limits_{k = 1}{n}(Y_{k} - \overline{Y}_{m})^{2}} \sim F(n - 1; m - 1)$$
...
***Точечной оценкой*** неизвестного параметра $\theta$ называется произвольная статистика 
$\hat{\theta}(Z_{n})$.
...
1. Из свойств $\overline{X}_{n}$ следует, что $\overline{X}_{n}$ --- **несмещенная** и
**сильносостоятельная** оценка $m_{X}$.

2. Из свойств $\hat{d}_{X}$ следует, что $\hat{d}_{X}$ --- **смещенная** и **сильносостоятельная** 
оценка $m_{X}$.

## Определение 10

*Несмещенная* оценка $\hat{\theta}^{*}(Z_{n})$ называется **эффективной**, если $\forall \hat{\theta}(Z_{n})$
...
Пусть $\hat{\theta}_{1}, \hat{\theta}_{2}$, --- *с.к.-оптимальные* оценки параметра $\theta$. Тогда 

$$\hat{\theta}_{1} = \hat{\theta}_{2}$$

### Доказательство {-}

Т.к. $\hat{\theta}_{1}$ и $\hat{\theta}_{2}$ **оптимальны**, то $D[\hat{\theta}_{1}] = D[\hat{\theta}_{2}] =
d$. 

Пусть $\hat{\theta}_{3} = \frac{1}{2} (\hat{\theta}_{1} + \hat{\theta}_{2})$. Тогда

$D[\hat{\theta}_{3}] = \frac{1}{4} D[\hat{\theta}_{1} + \hat{\theta}_{2}] = \frac{1}{4}
(D[\hat{\theta}_{1}] + [D\hat{\theta}_{2}] + 2\mathrm{cov}(\hat{\theta}_{1}, \hat{\theta}_{2})) =
\frac{1}{2}(d + \mathrm{cov}(\hat{\theta}_{1}, \hat{\theta}_{2})) \leqslant 
\frac{1}{2}(d + |\mathrm{cov}(\hat{\theta}_{1}, \hat{\theta}_{2})|)
\leqslant \frac{1}{2}(d + \sqrt{D[\hat{\theta}_{1}] \cdot D[\hat{\theta}_{2}]}) = d$

Тогда *в силу оптимальности* $\hat{\theta}_{1}$ и $\hat{\theta}_{2}$ 

$$D[\hat{\theta}_{3}] = d$$

$d = D[\hat{\theta}_{3}] = \frac{1}{2}(d + \mathrm{cov}(\hat{\theta}_{1}, \hat{\theta}_{2}))$, 
$\mathrm{cov}(\hat{\theta}_{1}, \hat{\theta}_{2}) = d = \sqrt{D[\hat{\theta}_{1}] \cdot D[\hat{\theta}_{2}]}$,

т.е. в *неравенстве Коши-Буняковского* достигается *равенство*. Следовательно,

$$\hat{\theta}_{1} = \alpha \hat{\theta}_{2} + \beta, \alpha > 0$$

$$\begin{cases}
    M[\hat{\theta}_{1}] = \alpha M[\hat{\theta}_{2}] + \beta \\
    D[\hat{\theta}_{1}] = \alpha^{2} D[\hat{\theta}_{2}] \\
\end{cases} \implies
\begin{cases}
...
Окончательно, $\hat{\theta}_{1} = \hat{\theta}_{2}$. $\blacksquare$
...
M[(\hat{\theta} - 0)(U(Z_{n}; \theta) - \theta)] + \theta \cdot M[U(Z_{n}; \theta)] =
...
\sum\limits_{k = 1}^{n} \frac{X_{k} - \theta}{\sigma^{2}} = \frac{n}{\sigma^{2}} (\overline{X}_{n} -
\theta)$

Т.о. $a(\theta) = \frac{\sigma^{2}}{n}$. Тогда $a(\theta) U(Z_{n}; \theta) = \overline{X}_{n} - \theta$, 
откуда следует, что $\overline{X}_{n}$ --- *эффективная оценка*.
...
множества** возможных значений $\theta$, то $\theta$ можно вычислить из *уравнения правдоподобия*
...
\frac{(X_{k} - \theta_{1})}{\theta_{2}} = \frac{1}{\theta_{2}}(n \overline{X}_{n} - n \theta_{1}) = 0 \\
    \frac{\partial \ln L(Z_{n}; \theta)}{\partial \theta_{2}} = \sum\limits_{k = 1}^{n}
(\frac{(X_{k} - \theta_{1})^{2}}{2 \theta_{2}^{2}} - \frac{1}{2 \theta_{2}})
= \frac{1}{2 \theta_{2}}(n \hat{d}_{X}(n) - n \theta_{2}) = 0 \\
\end{cases}$

$\begin{cases}
    \hat{\theta}_{1} = \overline{X}_{n} \\
    \hat{\theta}_{2} = \hat{d}_{X}(n) \\
\end{cases}$
...
Тогда оценка максимального правдоподобия **не может быть вычислена из уравнений правдоподобия**. Хотя
$\hat{\theta}$ существует:
...
Тогда $\hat{\theta}_{1} = \min_{k = \overline{1, n}} X_{k} = X^{(1)}, 
\hat{\theta}_{1} = \max_{k = \overline{1, n}} X_{k} = X^{(n)}$
...
$$\hat{g}(Z_{n}) = 
\begin{pmatrix}
    \frac{1}{2} + \Phi_{0}(\frac{x_{0} - \overline{X}_{n}}{\sqrt{\hat{d}_{X}(n)}}) \\
    \sqrt{\hat{d}_{X}(n)} \\
\end{pmatrix}$$
...
$\int\limits_{\R} g_{1}(x)dx < \infty, \int\limits_{\R} g_{2}(x)dx < \infty, 
...
$$\hat{\theta}_{i} = \varphi_{i}(\hat{\nu}_{1}(n), \dots, \hat{\nu}_{r}(n)), i = \overline{1, r}$$

называется ***оценкой метода моментов***.

## Теорема 2 {#theorem-9-2}

Пусть функции $\varphi_{1}, \dots, \varphi_{r}$, определяющие оценку метода моментов *непрерывные* и 
...
$X \sim N(\theta_{1}; \theta_{2})$. Тогда $\hat{\theta}_{1} = \overline{X}_{n}, 
\hat{\theta}_{2} = \hat{d}_{X}(n)$
...
$$\begin{cases}
    \overline{X}_{n} = \frac{\theta_{1} + \theta_{2}}{2} \\
    \hat{d}_{X}(n) = \frac{(\theta_{2} - \theta_{1})^{2}}{12} \\
\end{cases}
\begin{cases}
    \theta_{1} + \theta_{2} = 2 \overline{X}_{n} \\
    \theta_{2} - \theta_{1} = 2 \sqrt{\hat{d}_{X}(n)}\sqrt{3} \\
\end{cases}
\begin{cases}
    \theta_{1} = \overline{X}_{n} - \sqrt{3 \hat{d}_{X}(n)} \\
    \theta_{2} = \overline{X}_{n} + \sqrt{3 \hat{d}_{X}(n)} \\
\end{cases}$$
...
$$P(\hat{\theta}_{1}(Z_{n}) \leqslant \theta \leqslant \hat{\theta}_{2}(Z_{n})) = 1 - \alpha$$

для некоторого распределения параметром $F_{x}(x; \theta)$. Тогда интервал $[\hat{\theta}_{1}(Z_{n}); 
\hat{\theta}_{2}(Z_{n})]$ --- называется ***доверительным интервалом*** или ***интервальной оценкой***
...
$$P(\theta \leqslant \hat{\theta}_{1}(Z_{n})) = P(\theta \geqslant \hat{\theta}_{2}(Z_{n})) = 
\frac{\alpha}{2}$$
...
$P(\theta \geqslant \hat{\theta}_{2}(Z_{n})) = \alpha$ или 
$P(\theta \leqslant \hat{\theta}_{1}(Z_{n})) = \alpha$
...
$$P(G^{-1}(Z_{n}; g_{1}) \leqslant \theta \leqslant G^{-1}(Z_{n}; g_{2}) = 1 - \alpha$$

Тогда границы интервальной оценки имеют вид:

$$\hat{\theta}_{1}(Z_{n}) = G^{-1}(Z_{n}; g_{1}), \hat{\theta}_{2}(Z_{n}) = G^{-1}(Z_{n}; g_{2})$$

## Замечание {-}

Построение центральных статистик **довольно сложно**. В связи с чем зачастую используют**
...
1. $\frac{\overline{X}_{n} - m_{X}}{\sigma_{X}} \sqrt{n} \sim N(0; 1)$

2. $\frac{n \hat{d}_{X}(n)}{\sigma_{X}^{2}} \sim \chi(n - 1)$

3. $\frac{\overline{X}_{n} - m_{X}}{\sqrt{\hat{d}_{X}(n)}} \sqrt{n - 1} \sim t(n - 1)$

4. $\overline{X}_{n}$ и $\hat{d}_{X}(n)$ **независимы**

### Доказательство {-}

<!-- check here for sign in brackets i \ sigma sqrt n ... "_" -->
.1) $\frac{\overline{X}_{n} - m_{X}}{\sigma_{X}} \sqrt{n} = \sum\limits_{k = 1}^{n} \frac{1}{\sigma_{X}
...
$\frac{\overline{X}_{n} - m_{X}}{\sigma_{X}} \sqrt{n} \sim N(m; \sigma^{2})$, где $m = 
...
$\frac{n \hat{d}_{X}(n)}{\sigma_{X}^{2}} = \frac{1}{\sigma_{X}^{2}} \sum\limits_{k = 1}^{n}(X_{k} - 
\overline{X}_{n})^{2} = \frac{1}{\sigma_{X}^{2}} \sum\limits_{k = 1}^{n}(X_{k}^{2} - 
2 X_{k}\overline{X}_{n} + \overline{X}_{n}^{2}) = \frac{1}{\sigma_{X}^{2}}
(\sum\limits_{k = 1}^{n} X_{k}^{2} - 2n \overline{X}_{n}(\frac{1}{n}  \sum\limits_{k = 1}^{n} X_{k}) +
n \overline{X}_{n}^{2}) = \frac{1}{\sigma_{X}^{2}} \sum\limits_{k = 1}^{n} X_{k}^{2} - 
\frac{n}{\sigma_{X}^{2}} \overline{X}_{n}^{2} \overset{*}{=}$

$Y_{1} = \frac{1}{\sigma_{X} \sqrt{n}}(X_{1} + \dots + X_{n}) = \frac{\sqrt{n}}{\sigma_{X}} 
\overline{X}_{n}$. Тогда с учетом [Леммы 1](#lemma-10-1)
...
$$\frac{n \hat{d}_{X}(n)}{\sigma_{X}^{2}} = \sum\limits_{k = 2}^{n} Y_{k}^{2} \sim \chi^{2}(n - 1)$$

.4) Заметим, что $\overline{X}_{n} = \frac{\sigma_{X}}{\sqrt{n}} Y_{1} = \varphi(Y_{1})$, где 
$Y_{1}$ и $(Y_{2}, \dots, Y_{n})^{T}$ **независимы**. Тогда **независимы** также $\varphi(Y_{1})$ и
$\psi(Y_{2}, \dots, Y_{n})$, т.е. $\overline{X}_{n}$ и $\hat{d}_{X}(n)$

.3) $\frac{\overline{X}_{n} - m_{X}}{\sqrt{\hat{d}_{X}(n)}} \sqrt{n - 1} = 
...
$$G(Z_{n}; \theta) = \frac{\overline{X}_{n} - \theta}{\sqrt{\hat{d}_{X}(n)}} \sqrt{n - 1} \sim (n - 1)$$

Тогда 

$1 - \alpha = P(t_{\frac{\alpha}{2}}(n - 1) \leqslant \frac{\overline{X}_{n} - \theta}
{sqrt{\hat{d}_{X}(n)}}\sqrt(n - 1) \leqslant t_{1 - \frac{\alpha}{2}}(n - 1)) = 
P(\overline{X}_{n} - \sqrt{\frac{\hat{d}_{X}(n)}{n - 1}} t_{1 - \frac{\alpha}{2}}(n - 1) \leqslant \theta 
\leqslant \overline{X}_{n} + \sqrt{\frac{\hat{d}_{X}(n)}{n - 1}} t_{1 - \frac{\alpha}{2}}(n - 1))$

Тогда интервальная оценка имеет вид:

$$[\overline{X}_{n} - \sqrt{\frac{\hat{d}_{X}(n)}{n - 1}} t_{1 - \frac{\alpha}{2}}(n - 1); 
\overline{X}_{n} + \sqrt{\frac{\hat{d}_{X}(n)}{n - 1}} t_{1 - \frac{\alpha}{2}}(n - 1)]$$

Заметим, что $\overline{X}_{n}$ --- *точечная оценка* $\theta$ --- лежит в середине интервала. При этом
длина интервала $2 \sqrt{\frac{\hat{d}_{X}(n)}{n - 1}} t_{1 - \frac{\alpha}{2}}(n - 1)$ **возрастает** по 
...
\begin{cases}
    ...
    0^{n}, & 0 \leqslant 0, \\
    -(\frac{\theta \sqrt[n]{x} - 0}{\theta - 0})^{n}, & 0 \leqslant (0; \theta) \\
\end{cases} = 
\begin{cases}
    1, & x \geqslant 1, \\
    0, & x \leqslant 0, \\
...
***Статистической гипотезой*** $H$ называется любое предположение относительно закона распределения с.в.
...
***Статистическим критерием*** или ***критерием согласия*** называется правило, в соответствии с которым
...
2. Для построения доверительной и критической областей необходимо знать
...
$$T(Z_{n}) = \sup\limits_{x \in \R}|\hat{F}_{n}(x) - F(x)|$$
...
При больших $n \in \N$ используют *асимптотическую аппроксимацию*:
...
$$T_{\chi^{2}}(Z_{n}) = n \cdot \sum\limits_{k = 0}^{l} \frac{(\hat{p}_{k} - p_{k})^{2}}
{p_{k}}$$

где $\hat{p}_{0} = \hat{p}_{l} = 0$.
...
$$T(Z_{n}) = \frac{(\overline{X}_{n} - m_{0})}{\sigma_{X}} \sqrt{n}, Law(T(Z_{n}) | H_{0}) = N(0; 1)$$
...
$$T(Z_{n}) = \frac{(\overline{X}_{n} - m_{0})}{\sqrt{\hat{d}_{X}(n)}} \sqrt{n - 1}, 
...
$$T(Z_{n}) = \frac{n \hat{d}_{X}(n)}{\sigma_{0}^{2}},
...
В рамках регрессионного анализа рассматривается задача восстановления зависимости $Y = \varphi(x)$ 
...
$$\varphi(x; \theta) = \theta_{1}\varphi_{1}(x) + \dots + \theta_{s}\varphi_{s}(x)$$
...
$$Y_{k} = \theta_{1}\varphi_{1}(x) + \dots + \theta_{s}\varphi_{s}(x_{k}) + \varepsilon_{k}, 
k = \overline{1, n}$$
...
$Y = X\theta + E$, где $X = 
\begin{pmatrix}
    \varphi_{1}(x_{1}) & \dots & \varphi_{s}(x_{1}) \\
    \vdots & \ddots & \vdots \\
    \varphi_{1}(x_{n}) & \dots & \varphi_{s}(x_{n}) \\
...
\underset{\theta}{argmin}(\sum\limits_{k = 1}^{n}(Y_{k} - \theta_{1}\varphi_{1}(x_{k}) - \dots - 
\theta_{s}\varphi_{s}(x_{k}))^{2})$$
...
(X^{T}X)^{-1}X^{T}X\theta + (X^{T}X)^{-1}X^{T} \cancelto{0}{M[E]} = \theta$
...
В силу [Леммы 1.4](#lemma-1-4) достаточно показать, что $\hat{\theta}$ и $\hat{E}$ *некоррелированы*.
...
$CE_{0} = (\tilde{\varepsilon}_{1}, \dots, \tilde{\varepsilon}_{n}) \sim N(0, CIC^{T}) = 
N(0; I)$
...
$$\frac{\hat{\theta}_{k} - \theta_{k}}{\lVert \hat{E} \rVert \sqrt{\alpha_{k}}} \cdot \sqrt{n - s} 
\sim t(n - s)$$
...
Из [Леммы 1](#lemma-12-1) следует, что $\hat{\theta}_{k} \sim N(\theta_{k}, \sigma^{2} \alpha_{k})$

Тогда 

$\frac{\hat{\theta}_{k} - \theta_{k}}{\sigma \sqrt{\alpha_{k}}} \sim N(0; 1)$

С учетом [определения распределения Стьюдента](#def-7-2) и [Леммы 5](#lemma-12-5), и [Леммы 6](#lemma-12-6)

$\frac{\hat{\theta}_{k} - \theta_{k}}{\lVert \hat{E} \rVert \sqrt{\alpha_{k}}} \cdot \sqrt{n - s} = 
\frac{\frac{\hat{\theta}_{k} - \theta_{k}}{\sigma \sqrt{\alpha_{k}}}}{\sqrt{\frac{1}{n - s} 
...
$\frac{\varphi(x; \hat{\theta}_{k}) - \varphi(x; \theta_{k})}{\lVert \hat{E} \rVert \sqrt{\alpha_{k}}} 
\cdot \sqrt{n - s} \sim t(n - s)$, где

$\alpha(x) = (\varphi_{1}(x), \dots, \varphi_{s}(x))(X^{T}X)^{-1}
\begin{pmatrix}
    \varphi_{1}(x) \\
    \vdots \\
    \varphi_{s}(x) \\
...
$$\varphi(x; \hat{\theta}) = \hat{\theta}\varphi_{1}(x) + \dots + \hat{\theta}_{s}\varphi_{s}(x) \sim N(\varphi(x; 
\theta); \alpha(x)\sigma^{2})$$

Тогда 

$\frac{\varphi(x; \hat{\theta}_{k}) - \varphi(x; \theta_{k})}{\sigma \sqrt{\alpha_{k}}} \sim N(0; 1)$. 

С учетом лемм 5 и 6, определения Стьюдента

$\frac{\varphi(x; \hat{\theta}_{k}) - \varphi(x; \theta_{k})}{\lVert \hat{E} \rVert \sqrt{\alpha_{k}}} =
\frac{\frac{\varphi(x; \hat{\theta}_{k}) - \varphi(x; \theta_{k})}{\sigma \sqrt{\alpha_{k}}}}{
...
\sigma^{2}, \varphi(x;\theta)$.

Для построения точеченой оценки $\theta_{1}, \dots, \theta_{s}$ и $\varphi(x;\theta)$ используется 
[Теорема 1](#theorem-12-1), для точечной оценки --- [Лемма 3](#lemma-12-3).

Зачастую в качестве $\varphi_{k}(x)$ рассматривают $\varphi_{k}(x) = x^{k - 1}$. Однако порядок многочлена $s$,
...
$$T(Y) = \frac{\hat{\theta}_{k}^{2}}{\alpha_{k} \lVert \hat{E} \rVert^{2}}(n - s)$$
