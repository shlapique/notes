...
p <- ggplot(data.frame(x = c(0, 5)), aes(x = x)) +
  stat_function(fun = df, args = list(
    df1 = n,
    df2 = m
  ), aes(colour = "n > 2"))
p <- p + geom_line(data.frame(x = c(0, 0), f = c(0, break_y)), mapping = aes(
  x = x,
  y = f
), arrow = arrow(length = unit(0.01, "npc"), type = "closed"))
p <- p + geom_line(data.frame(x = c(0, 6), f = c(0, 0)),
  mapping = aes(x = x, y = f),
  arrow = arrow(length = unit(0.01, "npc"), type = "closed")
)
...
p <- p + geom_line(data.frame(x = c(break_x, break_x), f = c(0, break_y)), mapping = aes(
  x = x,
  y = f
), linetype = "dashed")
