...
.4) Заметим, что $\overline{X}_{n} = \frac{\sigma_{X}}{\sqrt{n}} Y_{1} = \phi(Y_{1})$, где 
...
$\frac{\phi(x; \hat{\theta}_{k}) - \phi(x; \theta_{k})}{\lVert \hat{E} \rVert \sqrt{\alpha_{k}}} 
...
$$\phi(x; \hat{\theta}) = \hat{\theta}\phi_{1}(x) + \dots + \hat{\theta}_{s}\phi_{s}(x) \sim N(\phi(x; 
\theta); \alpha(x)\sigma^{2})$$

Тогда 

$\frac{\phi(x; \hat{\theta}_{k}) - \phi(x; \theta_{k})}{\sigma \sqrt{\alpha_{k}}} \sim N(0; 1)$. 

С учетом лемм 5 и 6, определения Стьюдента

$\frac{\phi(x; \hat{\theta}_{k}) - \phi(x; \theta_{k})}{\lVert \hat{E} \rVert \sqrt{\alpha_{k}}} =
\frac{\frac{\phi(x; \hat{\theta}_{k}) - \phi(x; \theta_{k})}{\sigma \sqrt{\alpha_{k}}}}{
